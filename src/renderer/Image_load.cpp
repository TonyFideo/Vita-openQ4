...
#if defined(VITA) || defined(__vita__)
		VitaLoadingHud_SetAssetPhase( GetName(), "CACHE found" );
#endif

		// Validate the cached payload itself before doing any source probing.
		// This catches incompatible renderer formats and policy/usage changes even
		// on Vita where we deliberately avoid the expensive timestamp traversal.
		if ( !R_BinaryImageHeaderSupportedByRenderer( im.GetFileHeader() ) ) {
#if defined(VITA) || defined(__vita__)
			VitaLoadingHud_SetAssetPhase( GetName(), "CACHE format reject" );
#endif
			im.Clear();
			return false;
		}
...
#if defined(VITA) || defined(__vita__)
			VitaLoadingHud_SetAssetPhase( GetName(), "CACHE opts reject" );
#endif
			im.Clear();
			return false;
		}
...
#if defined(VITA) || defined(__vita__)
		// Vita runtime caches are generated by this port into fs_savepath. Once a
		// .bimage has parsed successfully and its renderer-facing header still
		// matches the requested policy, use it directly. Re-probing the original
		// image program merely to compare timestamps causes a very large VFS walk
		// (DDS/TGA variants across every search root and PK4). Vita3K has been
		// terminating host-side during that validation walk for
		// gfx/lights/squarelight1, despite the generated cache already being valid.
		//
		// A first load still takes the normal source/decode path and writes the
		// cache. Developers replacing loose source assets can invalidate it by
		// deleting data/Vita-OpenQ4/*/generated.
		VitaLoadingHud_SetAssetPhase( GetName(), "CACHE trusted" );
		return true;
#else
		if ( !productionMode ) {
