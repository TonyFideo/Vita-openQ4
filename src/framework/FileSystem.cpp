...
#if defined(VITA) || defined(__vita__)
	// The Vita GLES_D3 port bundles the core openQ4 runtime pak0 inside the
	// read-only application package. pak1 is ~2.8 GiB of ModernGL/light-grid
	// content and is deliberately omitted because those renderer paths are
	// unavailable on Vita. Packaging already validates pak0's structure.
	pack_t *const pak0 = FindGamePackByName( "pak0.pk4", OPENQ4_GAMEDIR );
	if ( pak0 == NULL || pak0->numfiles <= 0 ) {
		errors += va( "missing bundled %s/pak0.pk4\n", OPENQ4_GAMEDIR );
		return false;
	}
	return true;
#else
...
#else
	// Desktop keeps the original minizip state as a template and clones it.
