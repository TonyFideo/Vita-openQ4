...
- A release package that matches your operating system and CPU architecture (`x64` or `arm64`); Linux ARM64 downloads are currently preview packages, and Windows ARM64 downloads are experimental
...
| Operating system | Windows 7 or later for x64 packages, or Windows on Arm for the experimental ARM64 package; a modern 64-bit Linux desktop userspace comparable to Ubuntu 24.04; preview Linux ARM64 on 64-bit little-endian AArch64 hardware; preview macOS 11 or later on Apple Silicon/arm64, which players have so far run only on current macOS; SteamOS 3.x on Steam Deck | Windows 10/11, a current Linux distribution or SteamOS release, or an up-to-date Apple Silicon macOS install |
...
- macOS support is a preview. Current packages are for Apple Silicon/arm64 Macs on macOS 11 or later. Players have run them on macOS 26, including the single-player campaign and multiplayer with sound, but nothing older than the current macOS has been tested. Intel Mac and universal2 packages are not published yet, and Rosetta is not a supported release target.
- Release runs with Apple Developer ID signing and notarization credentials publish signed/notarized OpenGL and Metal bridge DMGs. Those credentials are not set up yet, so current releases publish clearly labeled `-unsigned.tar.gz` archives instead.
...
- Expect problems if you try it. The Vulkan renderer is experimental on every platform, and on macOS only one player has reported it working. If it cannot start, openQ4 logs the reason and renders with OpenGL instead; if it only fails later, while creating the window, openQ4 stops with an error and the next launch uses OpenGL. To go back, run `r_renderApi gl` and restart. See [Display Settings](display-settings.md#vulkan-on-macos-through-moltenvk).
...
- For crashes like GitHub issue #73, attach full terminal output as text plus `openq4.log` and any `.ips` report. See [macOS Support Data](macos-support-data.md), and run `collect_macos_support_info.sh` from the package root when it is included.
